...
float heightMap(vec2 p) { 
    ...
    p -= p*step(c, 0.5)*2.; // Equivalent to: if (c<.5) p *= -1.;
...
vec3 getNormal(vec3 p, float edge, float crv) { 
...
void main(){
    ...
    vec3 rd = normalize(vec3(2.*gl_FragCoord.xy - viewport.xy, viewport.y));
	...
	FragColor = vec4(sqrt(clamp(col, 0., 1.)), 1.)*1.5;
